...
router.post('/findPassword',function (req, res, next) {
  ...
  //这个接口同时用于密码的重置，需要用户的登录
  if (req.body.repassword) {
    //当repassword存在时，需要验证其登录情况或者验证其code
    if (req.body.token) {
      //当存在code登录状态时，验证其状态
      if (!req.body.user_id) {
    ...
    //当repassword不存在时，这里只是验证mail和phone，返回验证成功提示和提交的字段，用于之后改密码的操作
    if (!req.body.username) {
...
router.post('/download', function (req, res, next) {
  if (!req.body.movie_id) {
      res.json({status: 1, message: "电影id 传递失败"})
  } else {
      movie.findById(req.body.movie_id, function (err, supportMovie) {
          movie.update({_id: req.body.movie_id}, {movieNumDownload: supportMovie.movieNumDownload + 1}, function(err) {
              ...
              res.json({status: 0, message: "下载成功", data: {num:supportMovie.movieNumDownload,download:supportMovie.movieDownload}})
          })
      })
  }
});
